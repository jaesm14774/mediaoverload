...
本目錄包含 MediaOverload 專案的各種使用範例，幫助快速上手和理解系統功能。
...
results = run_all_strategies_multi_news(
    ...
    workflows={
        ...
        'text2image2image': {
            'first_stage': 'z_image_plus_nova_model_anime.json',
            'second_stage': 'z_image_i2i_anime.json'
        }
    }
)
```
...
| 策略 | 說明 |
|------|------|
| Text2ImageStrategy | 文生圖 |
| Image2ImageStrategy | 圖生圖 |
| Text2Image2ImageStrategy | 文生圖 → 圖生圖 |
| Text2VideoStrategy | 文生影片 |
| Text2Image2VideoStrategy | 文生圖 → 圖生影片 |
| Text2LongVideoStrategy | 文生長影片（尾幀驅動） |
| StickerPackStrategy | 貼圖包生成 |

每個策略都包含自定義 Prompt 範例、從資料庫獲取 News 範例，以及依不同新聞繪製多次的範例。

**適用場景**：需要了解所有策略的使用方法、喜歡互動式開發環境、需要批量生成（30 張、50 張等）。
...
圖片預設保存在 `output_media/` 目錄，可通過返回值的 `media_files` 欄位查看路徑：
...
將工作流 JSON 文件放在 `configs/workflow/` 目錄，然後在生成時指定：
...
使用完整版的 ContentGenerationService：
...
- [所有策略完整範例](all_strategies_examples.ipynb)（推薦）
