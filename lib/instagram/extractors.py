...
def extract_media_v1(data):
    ...
    if "image_versions2" in media:
        ...
        # Fix: Add default scans_profile for each candidate to satisfy pydantic validation
        for candidate in media["image_versions2"]["candidates"]:
            if "scans_profile" not in candidate:
                candidate["scans_profile"] = "e15"  # Default value
            if "estimated_scans_sizes" not in candidate:
                candidate["estimated_scans_sizes"] = []  # Default empty list
